...
/**
 * Type used for handling entity tags updates.
 * @type {string}
 */
const UPDATE_ITEM_TAG = `${prefix}UPDATE_ITEM_TAG`;

/**
 * Type used for handling entity tags updates failure.
 * @type {string}
 */
const UPDATE_ITEM_TAG_FAILURE = `${prefix}UPDATE_ITEM_TAG_FAILURE`;

/**
 * Type used for handling entity tags updates success.
 * @type {string}
 */
const UPDATE_ITEM_TAG_SUCCESS = `${prefix}UPDATE_ITEM_TAG_SUCCESS`;
 ...
 * }}
 */
const deleteItemTagFailure = ({ data, status } = {}) => ({
 ...
 * }}
 */
const updateItemTagFailure = ({ data, status } = {}) => ({
 ...
 * @return {{type: string, data: *}}
 */
const updateItemTagSuccess = data => ({
...
/**
 * Logic used for handling entity category deletion.
 * @method
 */
...
/**
 * Logic used for handling entity tag deletion.
 * @method
 */
const deleteItemTagLogic = createLogic({
  type: [
    DELETE_ITEM_TAG,
  ],
  latest: true,
  async process(
    ...
    try {
      ...
      if (status === 200 || status === 204) {
        dispatch(deleteItemTagSuccess());

        if (onSuccess) {
          onSuccess();
        }
      } else {
        dispatch(deleteItemTagFailure(response));
 ...
 * @method
 */
const updateItemTagLogic = createLogic({
  type: [
    UPDATE_ITEM_TAG,
  ],
  latest: true,
  async process(
    ...
    try {
      ...
      if (status === 200 || status === 201) {
        dispatch(updateItemTagSuccess(data));

        if (onSuccess) {
          onSuccess();
        }
      } else {
        dispatch(updateItemTagFailure(response));
